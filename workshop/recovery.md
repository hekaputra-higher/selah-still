...
If a live edit fails, leave the worktree intact and open http://localhost:3001. No reset, checkout or installation required. To preserve and reset the live branch later: commit or stash the work deliberately, then create another worktree from 06-local-recovery.
...
| 06-local-recovery | Complete local preparation package       | Always use this for known-good recovery                                              |
...
The original 05-finished tag is preserved but contains the former dev module-format bug. The new tag includes the tested fix.
